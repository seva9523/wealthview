...
function normalizeBalance(balance, wallet, priceBook) {
  ...
  const priced = Number.isFinite(priceUSD) && priceUSD >= 0;
  const usdValue = priced ? amount * priceUSD : null;
  const symbol = code;
  const supportedStablecoin = !native && Object.prototype.hasOwnProperty.call(FIAT_PEGS, String(code).toUpperCase());
  const type = native ? 'native' : supportedStablecoin ? 'stablecoin' : 'classic';
  return {
    ...
    usdValue: priced ? Number(usdValue.toFixed(2)) : null,
    valueUsd: priced ? Number(usdValue.toFixed(2)) : null,
    priced
  };
}
...
function mergeAssets(holdings) {
  ...
  for (const holding of holdings) {
    const existing = byKey.get(holding.key) || {
      ...
      priceUsd: holding.priceUsd,
      pricingSource: holding.pricingSource || holding.priceSource,
      priceSource: holding.priceSource || holding.pricingSource,
      usdValue: 0,
      valueUsd: 0,
      priced: holding.priced,
      wallets: []
    };
    existing.balance += Number(holding.balance || holding.amount || 0);
    if (holding.priced && Number.isFinite(holding.usdValue ?? holding.valueUsd)) {
      existing.usdValue += holding.usdValue ?? holding.valueUsd;
      existing.valueUsd += holding.usdValue ?? holding.valueUsd;
    }
  ...
  return [...byKey.values()].map((asset) => ({
    ...asset,
    amount: Number(asset.balance.toFixed(7)),
    balance: Number(asset.balance.toFixed(7)),
    usdValue: asset.priced ? Number(asset.usdValue.toFixed(2)) : null,
...
export async function aggregateWallets(walletInput = '', options = {}) {
  ...
  if (!requestedWallets.length) {
    return {
      ...
      version: '2026-06-wealthview-input-v1',
      totalXLM: 0,
      totalUSD: 0,
      pricedAssets: [],
      unpricedAssets: [],
  ...
  const totalUSD = Number(assets.reduce((sum, asset) => sum + (Number.isFinite(asset.usdValue) ? asset.usdValue : 0), 0).toFixed(2));
  const totalXLM = Number(assets.filter((asset) => asset.type === 'native' && asset.code === 'XLM').reduce((sum, asset) => sum + asset.amount, 0).toFixed(7));
  for (const asset of assets) {
    const allocationPercent = totalUSD > 0 && Number.isFinite(asset.usdValue) ? Number(((asset.usdValue / totalUSD) * 100).toFixed(2)) : 0;
    asset.allocationPercent = allocationPercent;
    asset.allocationPct = allocationPercent;
  }
